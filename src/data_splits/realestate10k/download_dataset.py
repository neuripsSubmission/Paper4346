...
Copyright (c) 2020 cashiwamochi

https://github.com/cashiwamochi/RealEstate10K_Downloader/blob/master/LICENSE
"""
...
def process(data, seq_id, videoname, output_root):
    ...
    # remove image folder
    command = "rm -rf " + output_root + "/" + seqname
    os.system(command)

    traj_name = seqname
    filePath = "/srv/share/datasets/RealEstate10k/features/" + traj_name + ".pt"
...
class DataDownloader:
    def __init__(self, dataroot, names, mode="train"):
        print("[INFO] Loading data list ... ", end="")
        self.dataroot = dataroot
        self.list_seqnames = names
        self.output_root = "/srv/share/datasets/RealEstate10k/dataset/" + mode + "/"
        self.mode = mode
        ...
        if not self.isDone:
            for txt_file in self.list_seqnames:
                ...
                seq_file = open(dataroot + txt_file, "r")
            ...
            print(" Done! ")
            print(
                "[INFO] {} movies are used in {} mode".format(
                    len(self.list_data), self.mode
                )
            )
    ...
    def Run(self):
        ...
        for data in self.list_data:
            ...
            #             wrap_process,
            #             [
            #                 (data, seq_id, videoname, self.output_root)
...
if __name__ == "__main__":
    ...
    good_trajs = sorted(msgpack_numpy.unpack(open(file, "rb"), raw=False))
    for fname in good_trajs:
        f = open(base_dir + fname, "r")

    mode = "sample"
    dataroot = "/srv/share/datasets/RealEstate10k/RealEstate10K/" + mode
    downloader = DataDownloader(base_dir, good_trajs, "train")
