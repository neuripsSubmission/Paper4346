...
class Agent:
    def __init__(
        ...
        # Params
        self.point_nav = args.point_nav
        self.validity_prediction = args.validity_prediction
        self.feat_prediction = args.feat_prediction
    ...
    def localize(self, test_position, test_rotation, skip=[], full_graph=False):
        ...
        for i in self.graph:
            ...
            if (euc_dist < 0.9 and (0 <= rot_dist < 29)) or (
                full_graph and euc_dist < 0.15 and (0 <= rot_dist < 30)
            ):
                return True, i
