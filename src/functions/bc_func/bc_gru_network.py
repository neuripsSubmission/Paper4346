...
class ActionNetwork(torch.nn.Module):
    def __init__(self):
        ...
        #     self.pos_encoder = PositionalEncoding(self.nhid, self.dropout_amnt)
        #     encoder_layers = TransformerEncoderLayer(32, nhead, 32, self.dropout_amnt)
    ...
    def forward(self, src, x1, x2):
        ...
        #     x = torch.cat((self.f1(x1), self.f1(x2), torch.mean(memory, dim=1)), 1)
        # else:  # self.type == "gru":
        src = self.encoder(src) * math.sqrt(self.nhid)
