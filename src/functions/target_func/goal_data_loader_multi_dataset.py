...
class Loader:
    def __init__(self, args):
        ...
        self.base_path = "/srv/flash1/mhahn30/topo_nav/gibson/"
        self.secondary_dist_dir = self.base_path + "distance_data_straight/"
        self.secondary_traj_dir = self.base_path + "trajectory_data/trajectoryInfo/"
        self.secondary_data = (
            "/nethome/mhahn30/Repositories/fair_internship/src/data_splits/gibson/"
        )

    def load_examples(self, splitScans, gibson_houses):
        node_feat1 = []
        node_feat2 = []
        ...
        for house in splitScans:
            ...
            if house in gibson_houses:
                distance_data_dir = self.secondary_dist_dir
                trajectory_info_dir = self.secondary_traj_dir
    ...
    def build_dataset(self, split):
        ...
        splitFile = self.args.data_splits + "scenes_" + split + ".txt"
        splitFile2 = self.secondary_data + "scenes_" + split + ".txt"
        splitScans2 = [x.strip() for x in open(splitFile2, "r").readlines()]
        splitScans.extend(splitScans2)
        (node_feat1, node_feat2, infos, switches) = self.load_examples(
            splitScans, splitScans2
        )
