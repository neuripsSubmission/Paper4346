...
class FMMPlanner:
    def __init__(self, traversible, num_rots, scale):
        ...
        # cv2.namedWindow('Path_scaled', cv2.WINDOW_NORMAL)
        # rotated = cv2.flip(self.traversible/(self.traversible.max()*1.), 0)
        # cv2.imshow('Path_scaled', rotated)
        # cv2.waitKey(1)

        self.angle_value = [0, 2.0 * np.pi / num_rots, -2.0 * np.pi / num_rots, 0]
        self.du = int(step_size / (self.scale * 1.0))
        self.num_rots = num_rots
        # self.mask = get_mask(0.5, 0.5, scale)
