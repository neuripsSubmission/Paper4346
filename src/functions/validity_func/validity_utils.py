...
def get_patch_coordinates(angles, goal, dist):
    coords = []
...
def get_labels(
    ...
    obstacle_threshold = 1.025
    pano_radius = map_size_cm / map_resolution / 2.0
    selem = skimage.morphology.disk(5 / map_resolution)
    goal_init = [pano_radius, pano_radius]
    smask = sector_mask(
        wall_map.shape,
        goal_init,
        pano_radius / radius,
    ...
    wall_map *= smask
    traversible = skimage.morphology.binary_dilation(wall_map, selem) != True
    planner = FMMPlanner(traversible, 360 // 15, 1)
    planner.set_goal(goal_init)

    labels = []
    coords = get_patch_coordinates(angles, goal_init, pano_radius / radius)
    for coord in coords:
        if (
            planner.fmm_dist[coord[0], coord[1]]
            < (pano_radius / 1.2) * obstacle_threshold
        ):
            label = 1.0
        else:
            label = 0.0
    ...
    smaller_dist = 2.4
    coords = get_patch_coordinates(angles, goal_init, pano_radius / smaller_dist)
    for enum, coord in enumerate(coords):
        if planner.fmm_dist[coord[0], coord[1]] < (pano_radius / smaller_dist) * 1.025:
            label = 1.0
        else:
            label = 0.0
        labels[enum] *= label
    ...
    # proj_map *= smask
    # exp_map *= smask
    # mapIm = cv2.flip(proj_map + 0.5 * exp_map, 0) * 255
    ...
    #     if labels[enum] == 0:
    #         continue
    #     color[coord[0], coord[1], :] = [0, 255, 0]
    #     color[coord[0], coord[1], :] = [0, 255, 0]
    #     color[coord[0], coord[1], :] = [0, 255, 0]
    #     color[coord[0], coord[1], :] = [0, 255, 0]
    # cv2.imwrite("test_depth.png", color)
